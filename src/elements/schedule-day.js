...
class ScheduleDay extends Element {
  static get template() {
    return `
        ...
        <template is="dom-repeat" items="[[timeslot.sessions]]" as="session" index-as="sessionIndex">
          ...
          <div class="session" style\$="grid-area: [[session.gridArea]]" layout="" vertical="">
            <template is="dom-repeat" items="[[session.items]]" as="subSession">
              <session-element class="subsession" day-name="[[name]]" session="[[subSession]]" user="[[user]]" featured-sessions="[[featuredSessions]]"></session-element>
            </template>
          </div>
